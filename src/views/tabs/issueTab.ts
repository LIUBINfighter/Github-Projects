...
export class IssueTab {
	...
	private showEmptyState(container: Element) {
		...
		emptyDiv.createEl("p", {
			text: "Add some repositories in the settings to get started.",
		});
	...
	private createStatsOverview(container: Element) {
		...
		// 计算总体统计
		const totalStats = this.repositories.reduce(
			(acc, repo) => {
	...
	private createRepositoryCard(container: Element, repo: RepositoryOverview) {
		...
		if (repo.issues.length > 0) {
			...
			repo.issues.slice(0, 3).forEach((issue) => {
				...
				const stateIcon = issueItem.createDiv(
					`issue-state issue-${issue.state}`
				);
				setIcon(
					stateIcon,
					issue.state === "open" ? "circle-dot" : "check-circle"
				);
	...
	private calculateStats(issues: GitHubIssue[]): WorkbenchStats {
		// const now = new Date(); // recentlyUpdated已注释
		...
		const diff =
			weekStart.getDate() - dayOfWeek + (dayOfWeek === 0 ? -6 : 1); // adjust when day is sunday
